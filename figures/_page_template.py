...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .cliptools{display:flex; gap:8px; align-items:center; flex-wrap:wrap;
             font-size:12.5px; color:var(--ink-2);
             background:var(--panel-2); border:1px solid var(--rule);
             border-radius:5px; padding:9px 11px;}
  .dlabel{font-family:"IBM Plex Mono",ui-monospace,monospace; font-size:10.5px;
  ...
  .slider{flex:1; min-width:200px; accent-color:var(--accent); height:26px;
          cursor:pointer;}
  .scrub .tstamp{white-space:nowrap; font-size:12.5px; color:var(--ink-2);}
  .marks{display:flex; gap:6px; align-items:center; flex-wrap:wrap; padding:0 2px;}
  .marks .mlab{font-family:"IBM Plex Mono",ui-monospace,monospace; font-size:10.5px;
               letter-spacing:.1em; text-transform:uppercase; color:var(--ink-3);
               margin-right:2px;}
  .mk{font:500 12px/1 "IBM Plex Mono",ui-monospace,monospace; cursor:pointer;
      background:var(--panel); color:var(--ink-2); border:1px solid var(--rule);
      border-radius:4px; padding:6px 10px;}
  .mk:hover{background:var(--accent-soft); color:var(--accent); border-color:var(--accent);}
  .mk.now{background:var(--accent); color:var(--panel); border-color:var(--accent);}
  .mk.p1{border-style:dashed;}
...
<header>
  ...
  <h1>Figures</h1>
  <p class="standfirst">Scene clips rebuilt from the gallery's own footage at 1920&times;1080,
  with the trajectory, per-gate scores and failure captions drawn in &mdash; scrub to any frame and
  download it. Below them, five analysis figures, each paired with the published precedent it
  copies, or with a note that no precedent exists.</p>
</header>
...
<section class="wide">
  ...
  &mdash; then re-composited against the corrected BEV.
  <strong>1920&times;1080 at 20&nbsp;fps, ~8.6&nbsp;s</strong>, against the gallery's
  1500&times;540 at 2&nbsp;fps for 3.5&nbsp;s.</p>
  <div class="callout">
    <h3>What the clip adds over the gallery version</h3>
    <ul class="measure">
      <li><strong>The trajectory is in the video.</strong> The gallery clips contain no
      trajectory lines at all &mdash; the viewer draws them in the browser, so the MP4 alone is
      nearly empty. Here the prediction is baked in, and projected into the camera image too.</li>
      <li><strong>Two phases.</strong> ~2&nbsp;s of history, cross-faded to 20&nbsp;fps instead of
      stuttering at 2, then a 4&nbsp;s future rollout where the camera holds and the BEV animates
      at 10&nbsp;Hz, with agent boxes interpolated between the nine stored timesteps.</li>
      <li><strong>Live state.</strong> Per-gate chips, a running clock, a scale bar, and the
      failure caption surfaced when it applies.</li>
    </ul>
  </div>
  <div class="clipgrid">
    <figure class="clipcard">
      <video class="clip" controls preload="none" playsinline
      ...
      <div class="scrub">
        <input class="slider" type="range" min="0" max="171" step="1" value="0"
               data-scrub aria-label="scrub to frame">
        <span class="tstamp" data-t>frame 0 / 171 &middot; t = 0.00 s</span>
      </div>
      <div class="marks" data-marks></div>
      <div class="cliptools">
        <span class="dlabel">Download this frame</span>
        <button class="btn" data-dl="cam">&#11015; Camera (left)</button>
  ...
  <div class="callout">
    <h3>Downloading a frame</h3>
    <p class="measure">Press play, pause where you want, then use the buttons under the video.
    The arrows step one frame (1/20&nbsp;s) at a time, and the readout shows the exact timestamp
    and frame index so a figure caption can cite it.</p>
    <ul class="measure">
      <li><strong>Camera (left)</strong> and <strong>BEV (right)</strong> save the two halves as
      separate PNGs, cropped at the renderer's own panel rectangles &mdash; the crop boxes come
      ...
      <li>The chips under the slider jump straight to a time point &mdash; dashed ones are in
      the replayed history (t&nbsp;&lt;&nbsp;0), solid ones in the future rollout. The chip for
      the segment you are in highlights as you scrub.</li>
      ...
      you were paused at.</li>
    </ul>
  </div>
  <p class="note">Ordered worst PDMS first. Posters are the t&nbsp;=&nbsp;+0.6&nbsp;s frame;
  nothing autoplays and nothing loads until you press play.</p>
</section>
...
SCRIPT = u"""
<script>
/* Frame grabber. Crops come from scenes_v2/clip_meta.json (written by the render job)
   so the panel rectangles are never duplicated here. MP4s are same-origin, so the
   canvas is not tainted and toBlob() works. Stepping is 1/20 s = the clips' frame rate. */
(function(){
  var FPS = 20, META = null, METAP = null;

  function meta(){
    if (METAP) return METAP;
    METAP = fetch("../scenes_v2/clip_meta.json")
      .then(function(r){ if(!r.ok) throw new Error(r.status); return r.json(); })
      .then(function(j){ META = j; return j; });
    return METAP;
  }
  function save(url, fname, revoke){
  ...
  function flash(btn, msg){
    ...
    btn.textContent = msg;
    setTimeout(function(){ btn.textContent = t; }, 1900);
  }
  function grab(v, rect){
    var c = document.createElement("canvas");
    var sx = rect ? rect[0] : 0, sy = rect ? rect[1] : 0;
    var sw = rect ? rect[2] : v.videoWidth, sh = rect ? rect[3] : v.videoHeight;
  ...
  function init(card){
    ...
    var marks = card.querySelector("[data-marks]");
    var NFRAMES = null, dragging = false, MARKS = [];
    ...
    function highlight(f){
      ...
      var best = 0;
      for (var i = 0; i < MARKS.length; i++) if (f >= MARKS[i].frame) best = i;
      for (var j = 0; j < MARKS.length; j++)
        MARKS[j].el.classList.toggle("now", j === best);
    }
    function show(){
    ...
    function goFrame(f){
      ...
      v.currentTime = Math.min(Math.max(f, 0) / FPS, d - 1e-3);
      if (scrub) scrub.value = String(f);
    }
    ...
    /* jump-to-time chips. Positions come from clip_meta.json (_meta.markers), written
       by the render job from the same constants it draws with -- not hardcoded here. */
    if (marks) meta().then(function(j){
      var list = (j._meta || {}).markers || [];
      if (!list.length) { marks.style.display = "none"; return; }
      var lab = document.createElement("span");
      lab.className = "mlab"; lab.textContent = "jump to";
      marks.appendChild(lab);
      list.forEach(function(m){
        var b = document.createElement("button");
        b.className = "mk" + (m.phase === 1 ? " p1" : "");
        b.type = "button";
        b.textContent = m.label;
        b.title = "real t = " + m.t + " s  \u00b7  frame " + m.frame;
        b.addEventListener("click", function(){ goFrame(m.frame); show(); });
        marks.appendChild(b);
        MARKS.push({frame: m.frame, el: b});
      });
      MARKS.sort(function(a, b2){ return a.frame - b2.frame; });
      show();
    }).catch(function(){ marks.style.display = "none"; });
    ...
    Array.prototype.forEach.call(card.querySelectorAll("[data-dl]"), function(b){
      b.addEventListener("click", function(){
        ...
        if (kind === "meta"){
          meta().then(function(j){
            var sc = (j.scenes || {})[tok];
            if (!sc) { flash(b, "no metadata"); return; }
            var out = JSON.parse(JSON.stringify(sc));
            out.frame_grabbed_at = {
              t_seconds: +(v.currentTime || 0).toFixed(3),
              frame_index: Math.round((v.currentTime || 0) * FPS),
              phase: (v.currentTime || 0) < 2.0 ? "history (t=-1.5..0.0s)"
                                                : "future rollout (t=0.0..4.0s)"
            };
            out._render = j._meta;
            var blob = new Blob([JSON.stringify(out, null, 2)],
                                {type: "application/json"});
            save(URL.createObjectURL(blob), tok + "_t" + stamp + "s.json", true);
          }).catch(function(){ flash(b, "metadata unavailable"); });
          return;
        }

        if (!v.videoWidth){ flash(b, "press play first"); return; }
        v.pause();
        var go = function(rect, suffix){
          var c;
          try { c = grab(v, rect); }
          catch (e) { flash(b, "grab blocked"); return; }
          var fname = tok + "_" + suffix + "_t" + stamp + "s.png";
          try {
            if (c.toBlob) c.toBlob(function(bl){
              if (bl) save(URL.createObjectURL(bl), fname, true);
              else save(c.toDataURL("image/png"), fname);
            }, "image/png");
            else save(c.toDataURL("image/png"), fname);
          } catch (e) { flash(b, "download blocked"); }
        };
        if (kind === "full"){ go(null, "full"); return; }
        meta().then(function(j){
          var r = ((j._meta || {}).panels || {})[kind];
          if (!r) { flash(b, "no crop box"); return; }
          go(r, kind);
        }).catch(function(){ flash(b, "metadata unavailable"); });
      });
    });
